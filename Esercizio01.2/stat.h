...
public:
	...
	void Set(const vector<double>&, unsigned int, unsigned int);
	void SetExpMean(double m) { exp_mean = m; }
	void Append(double x);
	//## Get Methods ##//
	Random* GetRandomGen() { return rnd; }
	vector<double> GetData() { return data; }
	unsigned int GetSampleSize() { return sample_size; }
	unsigned int GetDataSize() { return data.size(); } // Might be different from size!
	double GetExpMean() { return exp_mean; }
