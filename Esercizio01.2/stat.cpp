...
Stat :: Stat(const vector<double>& x, unsigned int _sample_size, unsigned int _blocks) {
	if (_sample_size < _blocks) {
...
void Stat :: SetData(const vector<double>& x) {
	data.clear();
	data = x;
	return;
}

void Stat :: Set(const vector<double>& x, unsigned int _sample_size, unsigned int _blocks) {
	data.clear();
	data = x;
	sample_size = _sample_size;
	blocks = _blocks;
