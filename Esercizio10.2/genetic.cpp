...
void Genetic :: OutputBestPath(const string filename) const { // Output the best path
	if (FinalPath.size() == 0) {
		cerr << "Error [Genetic :: OutputBestPath(const string)]: final path not set" << endl;
		exit(-1);
	}
	...
	if (out.is_open()) {
		...
		cerr << "Error [Genetic :: OutputBestPath(const string)]: cannot open input file" << endl;
		exit(-1);
	}
	return;
}

// Called after SORT has been invoked
void Genetic :: SetBest(int* B) {
	for (unsigned int n = 0; n < Nx; n++) {
		Population[Order[0]][n] = (unsigned int) B[n];
	}
	return;
}
int* Genetic :: GetBest() const {
	int* B = new int[Nx];
	for (unsigned int n = 0; n < Nx; n++) {
		B[n] = (int)Population[Order[0]][n];
	}
	return B;
}
