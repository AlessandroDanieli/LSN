...
public:
	...
	double GetBestL() const { return L[0]; }
	vector<double> GetL() const { return L; }
	Random* GetRandGen() const { return Rnd; }
	vector<Vector> GetX() const { return X; }
	...
	// Parallel computing methods (setting/getting the best path)
	void SetBest(int* B);
	int* GetBest() const;
};
