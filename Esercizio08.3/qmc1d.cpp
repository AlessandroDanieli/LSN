...
double variationalWaveFunction(double x) {
	double MeanMC = 0.811383;
	double SigmaMC = 0.620714;
	
	int c = 0;
	
	if (c == 0) return 1.;
	if (c == 1) {
		double diff = (x-MeanMC)/SigmaMC;
		double sum = (x+MeanMC)/SigmaMC;
...
double variationalWaveFunction_second(double x) {
	...
	if (c == 1) {
		double diff = (x-MeanMC)/SigmaMC;
		double sum = (x+MeanMC)/SigmaMC;
		return ((diff*diff-1.)*exp(-diff*diff/2.) + (sum*sum-1.)*exp(-sum*sum/2.))/(SigmaMC*SigmaMC);
	}
	return 0;
