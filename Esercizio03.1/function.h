...
public:
	...
	//virtual operator()(double x) const;
...
// Importance sampling: Normalized p(x) ~ mx+q
...
public:
	Sin();
	Sin(double _A, double _omega, double _phi);
	~Sin() {};
	...
	void SetPhi(double c) { phi = c; }
	
	double Eval(double x) const { return A*sin(omega*x + phi); }
	void Print() const;
};
