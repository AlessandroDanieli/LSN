...
double Random :: Gauss() { // ## Box-Muller Method, Standard Normal distribution N(0, 1) ## //
	double s=Uniform();
	double t=Uniform();
	return sqrt(-2.*log(1.-s))*cos(2.*M_PI*t);
}
...
// Inverse Sampling Method: F must be the inverse cumulative function of the wanted probability density
double Random :: InverseSampling(Function* inv_F) {
	return inv_F->Eval(Uniform());
}

// Classic Accept/Reject
double Random :: AR(Function* f, double xmin, double xmax, double ymax) {
	if (ymax <= 0) {
		cerr << "Error [Random :: AR(Function*, double, double, double)]: Invalid y maximum value" << endl;
		exit(-1);
	}
	double x, y;
	do {
...
// Hybrid Accept/Reject
double Random :: AR(Function* f, Function* inv_F) {
	double x, y;
	do {
		x = Uniform();
		y = inv_F->Eval(x);
