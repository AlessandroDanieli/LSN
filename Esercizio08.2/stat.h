...
public:
	...
	//## Data Analysis ##//
	double Average(double& mean) const;
	double AverageSigma(double& mean, double& sigma) const;
	double AverageVariance(double& mean, double& variance) const;
	
	// *** Average with its Standard Deviation or Variance *** //
	double BlockAverage(vector<double>& average) const; 
	double BlockSigma(vector<double>& sigma) const;
	double BlockVariance(vector<double>& variance) const;
	double BlockAverageSigma(vector<double>& average, vector<double>& sigma) const;
	double BlockAverageVariance(vector<double>& average, vector<double>& sigma) const;
	// Progressive mean and sigma of the mean (sigma/sqrt(N-1)) with Nblocks = Nsample
	double ProgAverageSigma(vector<double>& prog_average, vector<double>& prog_sigma) const;
	// Progressive average and its error for each block of data
	double BlockProgAverageSigma(vector<double>& prog_average, vector<double>& prog_sigma) const;

	double BlockProgAverageVariance(vector<double>& prog_average, vector<double>& prog_variance) const;
	
	// Returning only the last progressive average / sigma / variance
	double BlockProgAverageSigma(double& last_prog_average, double& last_prog_sigma) const;
	double BlockProgAverageVariance(double& last_prog_average, double& last_prog_variance) const; 
	...
	double BlockSigmaExp(vector<double>& sigma_exp) const;
	// Variance (using the expected mean exp_mean) for each block of data
	double BlockVarianceExp(vector<double>& variance_exp) const;
	// Progressive sigma and its error (using each block's average) for each block of data
	double BlockProgSigmaSigma(vector<double>& prog_sigma, vector<double>& prog_error) const;
	// Progressive variance and its error (using each block's average) for each block of data
	double BlockProgVarianceSigma(vector<double>& prog_variance, vector<double>& prog_error) const;
	// Progressive sigma and its error (using the expected mean exp_mean) for each block of data
	double BlockProgSigmaSigmaExp(vector<double>& prog_sigma, vector<double>& prog_error) const;
	// Progressive variance and its error (using the expected mean exp_mean) for each block of data
	double BlockProgVarianceSigmaExp(vector<double>& prog_variance, vector<double>& prog_error) const;
	
	// Returning only the last progressive average / sigma / variance
	double BlockProgSigmaSigma(double& last_prog_sigma, double& last_prog_error) const;
	double BlockProgVarianceSigma(double& last_prog_variance, double& last_prog_error) const;
	double BlockProgSigmaSigmaExp(double& last_prog_sigma, double& last_prog_error) const;
