...
Stat :: Stat(const vector<double>& x): rnd(nullptr) {
...
////// #####   StatIntegral   ##### //////////// #####   StatIntegral   ##### //////////// #####   StatIntegral   ##### //////
StatIntegral :: StatIntegral(): Stat(nullptr, 1, 0) {}
StatIntegral :: ~StatIntegral() {
	rnd->SaveSeed();
}

StatIntegral :: StatIntegral(Random* r, unsigned int size, unsigned int bl, Function* g, double a, double b) {
	rnd = r;
	sample_size = size;
	blocks = bl;
	f = g;
...
double StatIntegral :: FillLinear(double m, double q) {
	auto t1 = chrono::high_resolution_clock::now();
	data.clear();
	for (unsigned int i = 0; i < sample_size; i++) {
		data.push_back(rnd->Linear(xmin, xmax, m, q));
	}
	auto t2 = chrono::high_resolution_clock::now();
	return chrono::duration_cast<chrono::microseconds>(t2-t1).count();
}

// ## MonteCarlo integral (mean method) ## //
double StatIntegral :: BlockIntegrate() {
	auto t1 = chrono::high_resolution_clock::now();
	if (sample_size == 0) {
		cerr << "Error [Stat :: BlockIntegrate]: division by zero." << endl;
		exit(-1);
	}
	else {
		unsigned int L = (unsigned int) sample_size/blocks; //Lenght of each block of data
		...
		for (unsigned int n = 0; n < blocks; n++) {
			...
			for (unsigned int k = 0; k < L; k++) {
				integral[n] += f->Eval(data[n*L + k]);
			}
			integral[n] *= (double)(xmax - xmin) / L;
		}
	}
	auto t2 = chrono::high_resolution_clock::now();
	return chrono::duration_cast<chrono::microseconds>(t2-t1).count();
...
// ## Importance Sampling ## //
double StatIntegral :: BlockIntegrate(Function* p) {
	auto t1 = chrono::high_resolution_clock::now();
	if (sample_size == 0) {
		cerr << "Error [Stat :: BlockIntegrate(Function*)]: division by zero." << endl;
		exit(-1);
	}
	else {
		unsigned int L = (unsigned int) sample_size/blocks; //Lenght of each block of data
		...
		for (unsigned int n = 0; n < blocks; n++) {
			...
			for (unsigned int k = 0; k < L; k++) {
				integral[n] += f->Eval(data[n*L + k]) / p->Eval(data[n*L + k]); // Importance sampling f(x) / p(x)
			}
			integral[n] *= (double)(xmax - xmin) / L;
		}
	}
...
void StatIntegral :: PrintIntegral() const {
	cout << "Integral vector: " << endl;
	for (unsigned int i = 0 ; i < integral.size(); i ++) {
		cout << " #" << i+1 << " -> " << integral[i] << endl;
	}
	cout << "# # # # # # # # #" << endl;
	return;
}
