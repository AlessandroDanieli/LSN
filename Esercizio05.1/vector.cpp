...
Vector :: Vector(): V(nullptr), norm(-1), d(0) {}
...
//## Set Methods ##//
void Vector :: SetDim(unsigned int _d) {
	if (V == nullptr) {
		if (_d > 0) {
			d = _d;
			V = new double[d];
...
void Vector :: Set(vector<double> coord) {
	if (coord.size() == d) {
		for (unsigned int i = 0; i < d; i++) {
			V[i] = coord[i];
		}
		...
		if (coord.size() == 0) {
			...
			delete[] V;
			V = new double[d];
			for (unsigned int i = 0; i < d; i++) V[i] = coord[i];
			norm = -1;
			return;
		}
	}
...
void Vector :: SetSpherical(double radius, vector<double> angles) {
	if (radius < 0) {
		cerr << "Error [Vector :: SetSpherical(double, vector<double>)]: negative input radius" << endl;
		exit(-1);
	}
	if (angles.size()+1 != d) {
		d = angles.size()+1;
